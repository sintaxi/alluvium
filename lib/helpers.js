...
exports.normalize = function(analytics){
  ...
  Object.keys(analytics.devices || {}).forEach(function(key){
    device.push({ 
      name      : key,
      total     : analytics.devices[key]["total"],
      breakdown : analytics.devices[key]["breakdown"]
    })
  })

  Object.keys(analytics.os || {}).forEach(function(key){
    os.push({ 
      name      : key,
      total     : analytics.os[key]["total"],
      breakdown : analytics.os[key]["breakdown"]
    })
  })

  Object.keys(analytics.browser || {}).forEach(function(key){
    browser.push({ 
      name      : key,
      total     : analytics.browser[key]["total"],
      breakdown : analytics.browser[key]["breakdown"]
    })
  })
  ...
  return {
    ...
    encryption: {
      connEn: analytics.general.connEn,
      connUn: analytics.general.connUn,
      connR2En: analytics.general.connR2En,
      connR2Un: analytics.general.connR2Un
    },

    bandwidth: {
      all: analytics.general.bwT,
      body: analytics.general.bwB,
      headers: analytics.general.bwH,
    },

    cache: {
      hit: analytics.general.cacheHIT,
      miss: analytics.general.cacheMISS
    },
