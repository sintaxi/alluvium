var debug       = require("debug")("alluvium")
...
exports.createClient = function(cfg){
  ...
  return {
    ...
    write: function(data, callback){
      ...
      // ?: binds looser than || — the old unparenthesized form evaluated as
      // (data.browser || ua.browser.name) ? ... so a caller-supplied browser or
      // device (incl. the Deno label above) was always clobbered by the UA parse
      ...
      if (data){
        var leaderBoardConnectionKey = ["l", date, "tC"].join("::")
        transaction.zincrby(leaderBoardConnectionKey, 1, data.domain)
        transaction.expire(leaderBoardConnectionKey, pathsExpireInSeconds)
      }
       ...
       */
        ...
        var leaderBoardBAKey = ["l", date, "bA"].join("::")
        transaction.zincrby(leaderBoardBAKey, data.bwH + data.bwB, data.domain)
        transaction.expire(leaderBoardBAKey, pathsExpireInSeconds)
      }
      ...
      if (data.device){
        ...
        transaction.expire([data.domain, date, "uD"].join("::"), pathsExpireInSeconds)
      }
      ...
      if (data.referrer){
        ...
        // schemeless referrers ("example.com") parse with a null hostname and
        // the host in pathname; reparse host-first so hostname is populated
        // (otherwise sources record as "nullexample.com" and self-referrals
        // leak into rS instead of rM)
        if (!ref.hostname) ref = url.parse("//" + data.referrer, false, true)
      ...
      var trimPass = (Math.floor((Math.random() * 25) + 1)) === 1

      if (trimPass){
        debug("trimming", data.domain)
      ...
      transaction.exec(function(errors, replies){
        ...
        if (trimPass){
          // ZPOPMIN errors on a negative count so only trim sets over their limit
          var trims = [
            ["bF", replies[replies.length - 11] - 110],
            ["rS", replies[replies.length - 10] - 55],
            ["rM", replies[replies.length - 9] - 220],
            ["uD", replies[replies.length - 8] - 55],
            ["uO", replies[replies.length - 7] - 55],
            ["uB", replies[replies.length - 6] - 55],
            ["pS", replies[replies.length - 5] - 55],
            ["pR", replies[replies.length - 4] - 55],
            ["pN", replies[replies.length - 3] - 55]
          ]
          trims.forEach(function(trim){
            if (trim[1] > 0) trans.zpopmin([data.domain, date, trim[0]].join("::"), trim[1])
          })
        }
