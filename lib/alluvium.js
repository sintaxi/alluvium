  ...
  return {
    ...
    read: function(domain, options, callback){
      ...
      range.forEach(function(d){
        ...
        // General Counters
        transaction.zrevrange([domain, d, "general"].join("::"), 0, 15, "WITHSCORES")

        // User Agent
        transaction.zrevrange([domain, d, "uaDevice"  ].join("::"), 0, 10, "WITHSCORES")
        transaction.zrevrange([domain, d, "uaOS"      ].join("::"), 0, 10, "WITHSCORES")
        transaction.zrevrange([domain, d, "uaBrowser" ].join("::"), 0, 10, "WITHSCORES")
