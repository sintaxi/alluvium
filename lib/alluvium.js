...
exports.createClient = function(cfg){
  ...
  return {
    ...
    write: function(data, callback){
      ...
      if (data.deno){
        transaction.zincrby(generalKey, 1, "deC")
        transaction.expire(generalKey, expireInSeconds)
      }
      ...
      if (data.status == 200){
        if (
          ...
          transaction.expire([data.domain, date, "pSuccess"].join("::"), pathsExpireInSeconds)
        }
      }
      ...
      if (["301", "302", "303", "307", "308"].indexOf(data.status.toString()) !== -1){
        ...
        if (["connR2En", "connR2Un"].indexOf(data.encryption) === -1){
          ...
          transaction.expire([data.domain, date, "pRedirect"].join("::"), pathsExpireInSeconds) 
        }
      }
      ...
      if (data.status == 404){
        ...
        transaction.expire([data.domain, date, "pNotFound"].join("::"), pathsExpireInSeconds) 
      }
      ...
      if (data.browser){
        ...
        transaction.expire([data.domain, date, "browser"].join("::"), pathsExpireInSeconds)
      }

      if (data.os){
        transaction.zincrby([data.domain, date, "os"].join("::"), 1, data.os)
        transaction.expire([data.domain, date, "os"].join("::"), pathsExpireInSeconds)
      }
      ...
      if (data.filePath && data.bwB && data.bwB != 0){
        ...
        transaction.expire([data.domain, date, "bwFile"].join("::"), pathsExpireInSeconds)
      }
      ...
      if (data.referrer){
        ...
        if (ref.hostname === data.domain){
          ...
          transaction.expire([data.domain, date, "map"].join("::"), pathsExpireInSeconds)
        }else{
          transaction.zincrby([data.domain, date, "sources"].join("::"), 1, cleanRef)
          transaction.expire([data.domain, date, "sources"].join("::"), pathsExpireInSeconds)
        }
      }
      ...
      if (data.deno && data.ip){
        transaction.sadd([data.domain, date, "deip"].join("::"), data.ip)
        transaction.expire([data.domain, date, "deip"].join("::"), iPexpireInSeconds)
      }
      ...
      // get count of IPs to record Uniques
      transaction.scard([data.domain, date, "deip"].join("::"))
      ...
      transaction.exec(function(errors, replies){
        ...
        if (willTrim){
          trans.zpopmin([data.domain, date, "pNotFound"].join("::"), replies[replies.length - 11] - 55)
          trans.zpopmin([data.domain, date, "bwFile"].join("::"), replies[replies.length - 10] - 110)
          trans.zpopmin([data.domain, date, "sources"].join("::"), replies[replies.length - 9] - 55)
          trans.zpopmin([data.domain, date, "map"].join("::"), replies[replies.length - 8] - 220)
          trans.zpopmin([data.domain, date, "os"].join("::"), replies[replies.length - 7] - 55)
          trans.zpopmin([data.domain, date, "browser"].join("::"), replies[replies.length - 6] - 55)
          trans.zpopmin([data.domain, date, "devices"].join("::"), replies[replies.length - 5] - 55)
          trans.zpopmin([data.domain, date, "pSuccess"].join("::"), replies[replies.length - 4] - 55)
          trans.zpopmin([data.domain, date, "pRedirect"].join("::"), replies[replies.length - 3] - 55)
        }
        ...
        trans.zadd(generalKey, DenoUniques, "deU")
